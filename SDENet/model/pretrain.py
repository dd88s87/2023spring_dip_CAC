...
class MaskedAutoencoderViTNoCT(nn.Module):
    ...
    def __init__(self, img_size=384, embed_dim=768, patch_size=4, in_chans=3,
        ...
        # self.cov1 = nn.Conv2d(3, 3, patch_size, patch_size, 0)
        # img_size = img_size.to(torch.half)
        # patch_size = patch_size.to(torch.half)
        # in_chans = in_chans.to(torch.half)
        # embed_dim = embed_dim.to(torch.half)
        # print('type:', type(img_size),type(patch_size),type(in_chans),type(embed_dim))
        self.patch_embed = PatchEmbed(img_size, patch_size, in_chans, embed_dim)
        # self.patch_embed = PatchEmbed(384, 4, 3, 768)
