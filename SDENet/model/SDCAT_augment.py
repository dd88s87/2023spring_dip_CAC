...
class TransformerStage(nn.Module):
    def __init__(self, fmap_size, window_size, ns_per_pt,
        ...
        for i in range(depths):
            if stage_spec[i] == 'L':  # local attention
                self.attns.append(
                    LocalAttention(dim_embed, heads, window_size, attn_drop, proj_drop)
                )
            elif stage_spec[i] == 'D':  # deformable attention
                self.attns.append(
            ...
            elif stage_spec[i] == 'S':  # shift-window attention
...
class SDCAT(nn.Module):
    def __init__(self, img_size=384, embed_dim=768, patch_size=4, in_chans=3,
        ...
        for i in range(depth_dat):
            self.blocks.append(  # DAT
                # LocalAttention(embed_dim, heads, window_size, attn_drop, proj_drop)
                LocalAttention(embed_dim, 12, 6, 0., 0.)
            )
            self.blocks.append(
        ...
        # Exemplar encoder with CNN 可以改成resnet + DIP处理
    ...
    def augment(self, y1, N):
        ...
        for y in y1:
            ...
            B = torch.add(B, self.conv(y.unsqueeze(1)))
            f.append(self.conv(y.unsqueeze(1)))
        B /= len(y)
        s = 0
        for i in range(len(f)):
            t1 = torch.cat([f[i], B], dim=1)  # [N,32,512]
            t2 = torch.cat([self.conv2(f[i]), self.conv2(B)],dim=1)
            f[i] = torch.add(self.conv3(t1),t2)
            f[i] = self.mlp(f[i].view(N, -1)) # [N,512]
            f[i] = F.softmax(f[i], dim=1) # [N,512]
            s = torch.add(s, torch.mul(y1[i], f[i].to(y1[i].device)))
        for i in range(len(y1)):
            y1[i] = torch.add(y1[i], s)
        return y1
